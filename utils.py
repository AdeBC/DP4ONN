...
class species_tree(Tree):
    ...
    def from_paths(self, paths: list):
        ...
        stree.root = 'root'
        for path in paths:
            current_node = stree.root.id
	        for nid in path:
                children_ids = [n.id for n in stree.children(current_node)]
                if nid not in children_ids:
                    stree.create_node(nid.title(), nid, parent=current_node, data=0)
    ...
    def get_matrix(self, dtype=np.float32: object):
        paths_to_leaves = self.paths_to_leaves()
	    ncol = self.depth()
	    nrow = len(paths_to_leaves)
	    Matrix = zeros(ncol*nrow, dtype=dtype).reshape(nrow, ncol)
    ...
    def to_matrix_npy(self, file: str, dtype=np.float32: object ):
